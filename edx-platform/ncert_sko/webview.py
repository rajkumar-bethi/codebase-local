...
def _update_certificate_context(context, course, course_overview, user_certificate, platform_name, course_id):
    """
    Build up the certificate web view context using the provided values
    (Helper method to keep the view clean)
    """
    lms_site_url = settings.LMS_ROOT_URL
    frontend_site_url = settings.FRONTEND_URL
    context['lms_url'] = lms_site_url
    context['fe_url'] = frontend_site_url
...
def render_html_view(request, course_id, certificate=None):  # pylint: disable=too-many-statements
    ...
    with translation.override(certificate_language):
        ...
        _update_certificate_context(context, course, course_overview, user_certificate, platform_name, course_id)
        ...
        try:
            ...
            lms_site_url = settings.LMS_ROOT_URL
            img_path = os.path.join(settings.MEDIA_ROOT, f"certificate_image_{context['certificate_id_number']}.png")
            # generating image of certificate if not already exists
            if (not os.path.exists(img_path)):
                #logging.info(os.getcwd())
                html_template_path = "./lms/templates/certificates/certificate_template.html"
                with open(html_template_path, 'r', encoding='utf-8') as file:
                    html_context = file.read()
                html_context = html_context.replace('/static/', f'{lms_site_url}/static/')
                html_context = html_context.replace(f"User Name", f"{context['accomplishment_copy_name']}" )
                html_context = html_context.replace(f"Course Name", f"{context['accomplishment_copy_course_name']}" )
                html_context = html_context.replace(f"Issue Date", f"{context['certificate_date_issued']}" )
                html_context = html_context.replace(f"C13D-34CXVV-3423VB", f"{context['certificate_id_number']}" )

                # Dynamic organization logo insertion 
                organization_logo_html = "" 
                organization_long_name = context['organization_long_name'] 
                # List of valid organization names with logos 
                organizations_with_logos = ["Microsoft", "NASSCOM", "IBM", "Google Cloud"] 
                if organization_long_name in organizations_with_logos: 
                    organization_logo_html = f'<img src="{lms_site_url}/media/{context["organization_logo"]}" height="41" class="mr-8">' 
                else: 
                    organization_logo_html = '<img src="" height="41" class="mr-8">'
                html_context = html_context.replace('<img src="images/NTPC-logo.svg" width="74" height="41" />', organization_logo_html)
                imgkit.from_string(html_context, img_path)
                logging.info(f"Certificate image saved at: {img_path}")
                # Resize the certificate image to share on social media
                with Image.open(img_path) as img:
                    enhancer = ImageEnhance.Sharpness(img)
                    img = enhancer.enhance(2)  # Adjust enhancement factor as needed
                    img = img.resize((1200, 628), Image.LANCZOS)  # Resize for better clarity
                    img.save(img_path, dpi=(150, 150))  # Save with higher DPI for better quality

            if os.path.exists(img_path):
                img_path = f"{lms_site_url}/media/certificate_image_{context['certificate_id_number']}.png"
            else:
                img_path = f"/static/Openedx-theme/skillup_static/images/certificate.jpg"
            final_html_context = response.content.decode('utf-8')
            final_html_context = final_html_context.replace(
                    f"https://{settings.SITE_NAME}/certificates/",
                    f"{lms_site_url}/certificates/")
            updated_html_content = final_html_context.replace(
                '<meta property="og:image" content="{image_url}">',
                f'<meta property="og:image:url" content="{img_path}">'
            )
            response =  HttpResponse(updated_html_content)
